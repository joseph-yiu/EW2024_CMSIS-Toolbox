...
#if 	!HOST_EXAMPLE_CODE
#include "stdint.h"
extern void get_tick_count(uint32_t *tp);
extern void timer_start(void);
extern int stdio_init (void);
extern e_u32 verify_output;  //  allow overriding verify output at compile time

#define NSECS_PER_SEC 1000000000
#define EE_TIMER_TICKER_RATE 1000
#include <time.h>
#define ALTIMETYPE uint32_t 
#define GETMYTIME(_t) get_tick_count(_t)
#define MYTIMEDIFF(fin,ini) (fin-ini)
/* setting to 1/1000 of a second resolution by default with linux */
#ifndef TIMER_RES_DIVIDER
#define TIMER_RES_DIVIDER (NSECS_PER_SEC/EE_TIMER_TICKER_RATE)
#endif
ALTIMETYPE initial, final;
#endif
#if 	EE_POWERTAG
...
//#error "If not using host example code for timing, you must implement a method to measure time"
	...
	th_printf("Total allocated memory at signal finish: %.02fMB\n",(e_f32)th_malloc_total/(1024.0*1024.0));
#endif
  get_tick_count(&end_tick);
	th_printf("END: time=%d secs\n", end_tick/1000);
	return elapsed;
...
size_t al_ticks_per_sec( void )
{
	...
	/* Board specific timer code  */ 
//#error "If not using host example code for timing, you must implement a method to measure time"
			return (size_t) (NSECS_PER_SEC / TIMER_RES_DIVIDER);  // PORTME
