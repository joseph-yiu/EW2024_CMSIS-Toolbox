...
unsigned long get_100Hz_value(void)
{
  return ((0-IOTKIT_SECURE_TIMER0->VALUE)/(TIMER_CLK_SPEED/100));
}

#elif (defined (SSE300_CM55)||(SSE310_CM85))
...
unsigned long get_100Hz_value(void)
{
  struct arm_mps3_io_dev_t* arm_mps3_io_dev=&MPS3_IO_DEV;
  return (arm_mps3_io_read_clk100hz(arm_mps3_io_dev));
}
...
unsigned long get_100Hz_value(void)
{
  return ((0-CMSDK_TIMER0->VALUE)/(TIMER_CLK_SPEED/100));
}
